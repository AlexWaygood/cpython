...
the contents of :data:`sys.modules`, which is slightly unclean.    Holger Krekel's
...
:data:`sys.path` and then use the :option:`-m` switch to execute code from the
archive.
...
* Also deleted: the :file:`lib-old` directory, which includes ancient modules
  ...
  default :data:`sys.path`, so unless your programs explicitly added the
  directory to :data:`!sys.path`, this removal shouldn't affect your code.
...
* The Python developers switched from CVS to Subversion during the 2.5
  ...
  the :data:`sys.subversion` variable, a 3-tuple of ``(interpreter-name, branch-name,
