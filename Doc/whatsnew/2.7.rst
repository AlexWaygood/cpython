...
* New function: :func:`~runpy.run_path` in the :mod:`runpy` module
  ...
  :data:`sys.path` and the module :mod:`__main__` will be imported.  It's
  ...
  a location later in :data:`!sys.path`.  This makes more of the machinery
...
* The :mod:`syslog` module will now use the value of
  :data:`sys.argv[0] <sys.argv>` as the
  identifier instead of the previous default value of ``'python'``.
  (Changed by Sean Reifschneider; :issue:`8451`.)

* The :data:`sys.version_info` value is now a named tuple, with attributes
  named :attr:`~sys.version_info.major`, :attr:`~sys.version_info.minor`,
  :attr:`~sys.version_info.micro`, :attr:`~sys.version_info.releaselevel`, and
  :attr:`~sys.version_info.serial`.  (Contributed by Ross
  Light; :issue:`4285`.)

  :func:`sys.getwindowsversion` also returns a named tuple,
  with attributes named :attr:`~sys.getwindowsversion.major`,
  :attr:`~sys.getwindowsversion.minor`, :attr:`~sys.getwindowsversion.build`,
  :attr:`~sys.getwindowsversion.platform`,
  :attr:`~sys.getwindowsversion.service_pack`,
  :attr:`~sys.getwindowsversion.service_pack_major`,
  :attr:`~sys.getwindowsversion.service_pack_minor`,
  :attr:`~sys.getwindowsversion.suite_mask`, and
  :attr:`~sys.getwindowsversion.product_type`.
...
module into :data:`sys.modules` and returns the module object.
...
* New function: :c:func:`!PySys_SetArgvEx` sets the value of
  :data:`sys.argv` and can optionally update :data:`sys.path` to include the
  ...
  always update :data:`sys.path`, and sometimes it would add the current
...
* The path ``/Library/Python/2.7/site-packages`` is now appended to
  :data:`sys.path`, in order to share added packages between the system
  ...
  .. versionchanged:: 2.7.13
     ...
     :data:`sys.path` for user-installed Pythons such as from the python.org
