...
When a module does not define :attr:`~module.__loader__`, fall back to
...
* Starting in this release, there will be a concerted effort to begin
  ...
  takes care of for you), the :attr:`~module.__package__` attribute
  (superseded by
  :attr:`__spec__.parent <importlib.machinery.ModuleSpec.parent>`), the
  :attr:`~module.__loader__` attribute (superseded by
  :attr:`__spec__.loader <importlib.machinery.ModuleSpec.loader>`), and the
  :attr:`~module.__cached__` attribute (superseded by
  :attr:`__spec__.cached <importlib.machinery.ModuleSpec.cached>`) will slowly
  be removed (as well as other classes and methods in :mod:`importlib`).
...
* The import system now uses the :attr:`~module.__spec__` attribute on modules
  before falling back on :meth:`!module_repr` for a module's
