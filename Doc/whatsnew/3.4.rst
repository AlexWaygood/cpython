...
* Module :attr:`~module.__file__` attributes (and related values) should now always
...
``-m`` switch) to initialise :mod:`__main__` appropriately in child processes
...
* :meth:`!importlib.util.module_for_loader` now sets :attr:`~module.__loader__`
  and :attr:`~module.__package__` unconditionally to properly support
  reloading. If this is not
  desired then you will need to set these attributes manually. You can use
  :func:`importlib.util.module_to_load` for module management.

* Import now resets relevant attributes (such as :attr:`~module.__name__`,
  :attr:`~module.__loader__`, :attr:`~module.__package__`,
  :attr:`~module.__file__` and :attr:`~module.__cached__`) unconditionally when
  reloading.
  Note that this restores a pre-3.3 behavior in that it means a module is
  re-found when re-loaded (:issue:`19413`).

* Frozen packages no longer set :attr:`~module.__path__` to a list containing
  the package
...
* Frozen modules no longer define a :attr:`~module.__file__` attribute. It's
  semantically incorrect for frozen modules to set the attribute as they are
  not loaded from any explicit location. If you must know that a module comes
  from frozen code then you can see if the module's
  :attr:`__spec__.location <importlib.machinery.ModuleSpec.location>` is set to
...
* :class:`importlib.machinery.PathFinder` now passes on the current working
  ...
  will not find all keys. A module's :attr:`~module.__file__` when imported in
  the current working directory will also now have an absolute path, including
  when using ``-m`` with the interpreter (except for ``__main__.__file__`` when
  a script has been executed directly using a relative path). (Contributed by
  Brett Cannon in :issue:`18416`.)
...
* :mod:`doctest` now looks for doctests in extension module
  :attr:`~module.__doc__`
