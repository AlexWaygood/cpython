...
When performing a relative import, falling back on :attr:`~module.__name__` and
:attr:`~module.__path__` from the calling module when :attr:`~module.__spec__`
or :attr:`~module.__package__` are not defined now raises an
:exc:`ImportWarning`.
...
* When performing a relative import, if :attr:`~module.__package__` does not
  compare equal to
  :attr:`__spec__.parent <importlib.machinery.ModuleSpec.parent>` then
  :exc:`ImportWarning` is raised.
