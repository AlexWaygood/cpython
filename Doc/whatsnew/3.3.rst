...
attributes on :data:`sys.implementation` are :attr:`~sys.implementation.name`,
:attr:`~sys.implementation.version`, :attr:`~sys.implementation.hexversion`,
and :attr:`~sys.implementation.cache_tag`.

The intention of :data:`sys.implementation` is to consolidate into one namespace
...
much more easily.  In its initial state, :data:`!sys.implementation` holds only a
...
One example of improved standard library portability is :attr:`!cache_tag`.  As of
Python 3.3, :attr:`sys.implementation.cache_tag` is used by :mod:`importlib` to
support :pep:`3147` compliance.  Any Python implementation that uses
:mod:`!importlib` for its built-in import system may use :attr:`!cache_tag` to
control the caching behavior for modules.

SimpleNamespace
---------------

The implementation of :data:`sys.implementation` also introduces a new type to
...
by :data:`sys.stdout` if the console output code page is set to cp65001 (e.g.,
...
* The -Q command-line flag and related artifacts have been removed.  Code
  checking :attr:`!sys.flags.division_warning` will need updating.
