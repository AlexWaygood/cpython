...
:data:`sys.stdout` and then restore the old value.  For sending output to
...
mostly by Trent Mick of ActiveState.  (Confusingly, :data:`sys.platform` is still
...
:mod:`sys` module.  :data:`sys.version_info` is a tuple ``(major, minor, micro,
level, serial)`` For example, in a hypothetical 2.0.1beta1, :data:`!sys.version_info`
...
* :mod:`atexit`:  For registering functions to be called before the Python
  interpreter exits. Code that currently sets :func:`sys.exitfunc` directly should be
...
:file:`lib-old`, you can simply add that directory to :data:`sys.path` to get them
