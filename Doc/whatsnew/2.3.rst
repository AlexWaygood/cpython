...
automatically imported if a ZIP archive's filename is added to :data:`sys.path`.
For example:
...
An entry in :data:`sys.path` can now be the filename of a ZIP archive. The ZIP
...
* :data:`sys.path_hooks` is a list of callable objects; most often they'll be
...
* :data:`sys.path_importer_cache` caches importer objects for each path, so
  :data:`sys.path_hooks` will only need to be traversed once for each path.

* :data:`sys.meta_path` is a list of importer objects that will be traversed
  before :data:`sys.path` is checked.  This list is initially empty, but user
  code can add
...
* The value of the C :c:macro:`PYTHON_API_VERSION` macro is now exposed at the
  Python level as :data:`sys.api_version`.  The current exception can be
